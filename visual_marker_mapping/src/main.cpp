...
//------------------------------------------------------------------------------------------------------------
template<typename T>
void checkRange(T param, std::string paramName, T lowerBound = 0, T upperBound = std::numeric_limits<T>::max())
{
    if(param < lowerBound)
        throw std::runtime_error("Value for " + paramName + " is smaller then " + std::to_string(lowerBound) + " .");
    if(param > upperBound)
        throw std::runtime_error("Value for parameter " + paramName + " is bigger than " + std::to_string(upperBound) + " ." );
}
//------------------------------------------------------------------------------------------------------------
...
po::variables_map loadParameters(int argc, char *argv[])
{
    ...
    options.add_options()   ("help,?", "produces this help message")
                            ...
                            ("do_corner_refinment", po::bool_switch()->default_value(false), "If this option is set an additional corner refinment of the marker corners is done.");

    fileOptions.add_options()
                            ("camera_parameter_file", po::value<std::string>()->default_value(""), "Path to the xml file with the camera parameters.")
                            ("tag_img_path", po::value<std::string>()->default_value("."), "Path to the folder with the images of the tags.")
                            ("tag_detections_path", po::value<std::string>()->default_value("dbg_tag_detections"), "Path to the folder where the processed images are stored.")
                            ("reconst_json_filename", po::value<std::string>() -> default_value("reconstructedObjects.json"),"Filename of the file in which the json with reconstructed Tags and Cameras is stored. It is saved in the root folder.")
                            ("json_filename", po::value<std::string>() -> default_value("marker.json"),"Filename of the file in which ths json is stored. It is saved in the root folder.");
    ...
    fileOptions.add_options()
                            ...
                            ("tag_width",
                             po::value<double>()->default_value(0.116)
                                                ->notifier([](int param){
                                                    checkRange<double>(param, "tag_width", 0.0);
                                                }),
                             "Width of an tag measured parallel to the tag name.")
                            ("tag_height",
                             po::value<double>()->default_value(0.117)
                                                ->notifier([](int param){
                            ...
                            ("max_threads",
                             ...
                             "The number of threads with which the program is working. Currently this parameter is only used for Ceres.")
                            ("start_tag_id",
                             po::value<int>()->default_value(1)
                                             ->notifier([](int param){
    ...
    if(!ini_file)
        throw std::runtime_error("Could not open the config file: " + configFilePath);
...
int main(int argc, char *argv[])
{
    ...
    if(vm["do_reconstruction"].as<bool>())
    {
        ...
        const std::string jsonRecFilepath = projectPath + "/" + vm["reconst_json_filename"].as<std::string>();
    ...
    else
    {
        ...
        camSurv::TagDetector tagDetector(projectPath,
                                        imgPath, 
                                        tagDetectionsPath,
                                        vm["visualization_height"].as<int>(), 
                                        vm["visualization_width"].as<int>(),
                                        vm["tag_width"].as<double>(),
                                        vm["tag_height"].as<double>());
        
        const std::string imgFolder = projectPath + "/" + imgPath;
        const auto result = tagDetector.detectTags( imgFolder, 
                                                    vm["do_corner_refinment"].as<bool>(),
                                                    vm["show_tags"].as<bool>());
