...
void TagReconstructor::startReconstruction(int numThreads)
{
    ...
    {
        std::string errMsg = "Could not use tag with id " +std::to_string(originTagId) + " as origin tag, because it was not detected.";
    ...
    // Init with the first tag id. If no tag is reconstructed at the beginning this 
    // tag id will be taken to reconstruct the inital tag
    while (1)
    {
        std::cout << "Reconstructing image " << reconstructedCameras.size() << "/" << detectionResults_.images.size() << " | " << imageFilenames[curImageId] << " with id: " << curImageId << std::endl;
        ...
        {
            throw std::runtime_error("No reconstructed tags in image found. To reconstruct the image pose already reconstructed markers are needed. This should NOT happen.");
        }
        ...
        for (const auto& tagObs : detectionResults_.tagObservations)
        {
            ...
            if (0)
            {
                ...
                const double tagRatio = widthInImage < heightInImage ? widthInImage/heightInImage : heightInImage / widthInImage;
                if(tagRatio < 0.6)
                {
                    std::cout << "   Skipping reconstruction of tag " << tagObs.tagId << ": Bad aspect ratio: " << tagRatio << " < 0.6" << std::endl;
                    continue;
                }
            }
            if (whichImagesObserveTag[tagObs.tagId].size()<2)
            {
                std::cout << "   Skipping reconstruction of tag " << tagObs.tagId << ": Only observed once!" << std::endl;
                continue;
            }
            ...
            //std::cout << "Adding new tag with id " << tagId << " from camera " << camId << std::endl; 
            Eigen::Matrix3d R;
            Eigen::Vector3d t;
            ...
            //const Eigen::Matrix4d cameraTransformation = reconstructedCameras[curImageId].getCameraTransformation();
            Eigen::Matrix4d extrinsic = Eigen::Matrix4d::Identity();
            extrinsic.block<3,3>(0,0) = reconstructedCameras[curImageId].quat().toRotationMatrix();
        ...
        for (const auto& img : detectionResults_.images)
        {
            ...
            // look for an unreconstructed image with the maximum number of reconsturcted tags
...
//-----------------------------------------------------------------------------
bool TagReconstructor::computeRelativeCameraPoseFromImg(int imageId,
...
const std::map<int, double> TagReconstructor::computeReprojectionErrorPerImg() const
{
    ...
    for (const auto& tagObs : detectionResults_.tagObservations)
    {
        ...
        {
            const Eigen::Vector2d dist = camModel.projectPoint(R*pts3d[i]+t) - tagObs.corners[i];
            sum += dist.norm();
        }
    ...
    // set the reprojection error of cameras without any observations to something invalid
...
const std::vector<Eigen::Vector2d> TagReconstructor::computeReprojectionErrorPerCorner() const
{
    ...
    for (const auto& tagObs : detectionResults_.tagObservations)
    {
        ...
        for (size_t i = 0; i < pts3d.size(); ++i)
        {
            reprojectionErrors.push_back(camModel.projectPoint(R*pts3d[i]+t) - tagObs.corners[i]);
        }
    }
...
#endif
//-----------------------------------------------------------------------------
void TagReconstructor::doBundleAdjustment(int maxNumIterations,
                                          int ceresThreads, bool robustify,
                                          bool printSummary)
{
    std::map<int, std::set<int> > whichImagesObserveTag; // tagid -> [imageId]
    for (const auto& tagObs : detectionResults_.tagObservations)
    ...
    if (printSummary)
    {
        ...
        for (const auto& reconstTag : reconstructedTags)
        {
            ...
            covarianceComputation.GetCovarianceBlock(&reconstTag.second.t(0), &reconstTag.second.t(0), &covariance(0,0));
            ...
            std::cout << "StdDev of tag " << tagId << ": " << std_diag.transpose() << " | StdDevNorm: " << sqrt(std_diag.norm()) << std::endl;

            averageCovDiagonal+=diag;
        }
        averageCovDiagonal /= reconstructedTags.size();
        Eigen::Vector3d averageStdDevDiagonal;
        averageStdDevDiagonal << sqrt(averageCovDiagonal(0)), sqrt(averageCovDiagonal(1)), sqrt(averageCovDiagonal(2));
...
void TagReconstructor::removeBadMarkers(double threshold)
{
    ...
    for (const auto& tr : reperrors)
    {
        if (tr.second>threshold)
        {
            ...
            {
                std::cout << "Removing bad marker with id " << tr.first << " and reprojection error " << tr.second << std::endl;
...
void TagReconstructor::removeBadCameras(double threshold)
{
    ...
    for (const auto& tr : repErrors)
    {
        ...
        {
            std::cout << "Removing bad camera with id " << tr.first << " and reprojection error " << tr.second << std::endl;
