...
namespace camSurv
{
TagDetector::TagDetector(const std::string& rootPath,
...
//-------------------------------------------------------------------------------------------------
DetectionResult TagDetector::detectTags(const std::vector<std::string>& filePaths, bool doCornerRefinment, bool showMarkers)
{ 
    ...
    auto tagDetector=std::unique_ptr<AprilTags::TagDetector>(new AprilTags::TagDetector(AprilTags::tagCodes36h11));
    ...
    for (const auto& filePath : filePaths)
    {
        ...
        std::cout << "Processing file " << (imageId+1) << "/" << filePaths.size() << " " << filePath << std::endl;

        const cv::Mat img = cv::imread(filePath, CV_LOAD_IMAGE_GRAYSCALE); 
        
        if(img.cols != imgWidth ||
           img.rows != imgHeight) 
        {
            std::cerr << "Image " << p.filename() << " has not the correct size of "<< imgHeight << " x " << imgWidth << std::endl;
            continue; 
        }
        ...
        for (const auto& detectedTag : detectedTags)
        {
            ...
            // Debug Visualization
            const cv::Point pos((detectedTag.p[0].first + detectedTag.p[2].first)/2, (detectedTag.p[0].second +detectedTag.p[2].second)/2);

            cv::putText(visualization,
                        std::to_string(detectedTag.id),
                        pos,
            ...
            for (int i = 0; i < 4; ++i) 
            {
                const cv::Scalar colorMap[4]={cv::Scalar(255, 0, 0),cv::Scalar(0, 255, 0),cv::Scalar(0, 0, 255),cv::Scalar(255, 255, 0)};
                const cv::Scalar color = colorMap[i];

                cv::circle( visualization, 
                            cv::Point2f(tagObs.corners[i].x(), tagObs.corners[i].y()), 
                            img.cols * 0.002,
                            color,
                            10);  
            }
...
//-------------------------------------------------------------------------------------------------
DetectionResult TagDetector::detectTags(const std::string& folder, bool doCornerRefinment, bool showMarkers)
{
    std::regex reg("(.*)\\.((png)|(jpg))", std::regex_constants::ECMAScript |std::regex_constants::icase);
