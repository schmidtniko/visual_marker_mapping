...
struct TagReconstructionCostFunction {
    TagReconstructionCostFunction(const Eigen::Vector2d& observation,
            double tagWidth,
            double tagHeight,
            const Eigen::Matrix<double, 5, 1>& d,
            const Eigen::Matrix3d& K,
    ...
    bool operator()(const T* const camera_xyz,
                    const T* const camera_q,
                    const T* const tagPose_xyz,
                    const T* const tagPose_q,
                    T* residuals) const {
...
// The error is the difference between the predicted and observed position.
    ...
    static ceres::CostFunction* Create(const Eigen::Vector2d& observations,
            ...
            const Eigen::Matrix3d& K,
            const Eigen::Vector3d& tagCorner) {
      return (new ceres::AutoDiffCostFunction<TagReconstructionCostFunction, 2, 3, 4, 3, 4>(          //  residuals, camPose, tagPose
